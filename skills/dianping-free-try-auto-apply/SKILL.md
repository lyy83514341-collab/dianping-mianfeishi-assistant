...
- 成功后立即写状态并回到列表
- 验证码或未知状态一律暂停，不猜测
- 列表出现“当前活动太火爆”或“网络异常”时，按 5、10、20 秒退避重开三次；三次仍失败才允许切换详情兜底
- 重开列表时保留本批已处理卡片集合，并重置滚动和停滞计数，从顶部重新扫描而不重复提交
...
- 列表提示“当前活动太火爆”“网络异常”或其他仅首页故障，且退避重开仍未恢复
...
- 连续 3 个不同活动均返回已验证的 `detail_blank` 时暂停直达路线；自动模式仍执行唯一一次复扫并再次列表优先，之后明确报告未解决 ID，不把整个剩余批次机械跑两遍
- 第二轮将此前空白的 ID 排在尚未尝试的 ID 后面，防止固定空白前缀阻塞后续候选
